...
class NativeZipReader {
  ...
  static async loadAsync(input: any): Promise<NativeZipReader> {
    let buf: Buffer;
  ...
  file(name: string, data: any, _options: Record<string, any> = {}): this {
    this.files[name] = {
      name,
      dir: name.endsWith('/'),
      async: async () => data,
...
class NativeZipWriter {
  ...
  append(data: any, options: { name?: string; base64?: boolean } | string = {}): void {
    const name =
      typeof options === 'string' ? options : options && options.name ? options.name : 'file';
    let buf: Buffer;
    if (Buffer.isBuffer(data)) {
      buf = data;
    } else if (typeof data === 'string') {
      const isBase64 = typeof options === 'object' && options?.base64;
      buf = Buffer.from(data, isBase64 ? 'base64' : 'utf8');
    } else {
...
const JSZip = NativeZipReader;
export { NativeZipReader, NativeZipWriter, JSZip };
export default {
  NativeZipReader,
  NativeZipWriter,
